...
function generateAssetBundles(htmlFile) {
  ...
  if (styles.length) {
    const styleGroups = getResourceElementGroups(htmlFile, styles, styleElementSelector);
    const styleBundles = getStyleBundles(htmlFile, styleGroups);
    outputScripts = outputScripts.concat(styleBundles);
  }

  if (scripts.length) {
    const scriptGroups = getResourceElementGroups(htmlFile, scripts, scriptElementSelector);
...
function getResourceElementGroups(htmlFile, resourceElements, selector) {
  return resourceElements.reduce((acc, resourceElement) => {
    if (htmlFile.ch(resourceElement.el).prev().is(selector)) {
...
function bundleGroup({ htmlFile, group, index, filenameSuffix, fileExt, templateFn }) {
  ...
  const origAssetPath = path.join('/', htmlFile.assetAbsoluteWebDir, `${name}${fileExt}`);
  htmlFile.ch(group[0].el).replaceWith(templateFn(origAssetPath));
